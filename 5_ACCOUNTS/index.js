...
function errorMsg(text) {
  console.error(chalk.bgRed.bold(text));
}
...
function infoMsg(text) {
  console.log(chalk.bgBlue.bold(text));
}
...
function transfer() {
  ...
  inquirer
    .prompt([
      {
        name: "accountName",
        message: "Please, type your account name: ",
      },
    ])
    .then((answer) => {
      const sourceAccount = answer["accountName"];

      if (!checkAccount(sourceAccount)) {
        return transfer();
      }
      ...
      inquirer
        ...
        .then((answer) => {
          ...
          if(!checkAmount(amount, sourceAccountData.balance)) {
            return transfer();
          }
          ...
          inquirer
            .prompt([
              {
                name: "targetAccount",
                message: "Please type the account name you want to transfer.",
              },
            ])
            .then((answer) => {
              const targetAccount = answer["targetAccount"];
              const targetAccountData = getAccount(targetAccount);              
              ...
              // make a transfer
              sourceAccountData.balance =
                parseFloat(sourceAccountData.balance) - parseFloat(amount);
              targetAccountData.balance =
                parseFloat(targetAccountData.balance) + parseFloat(amount);
              ...
              fs.writeFileSync(
                `accounts/${targetAccount}.json`,
                JSON.stringify(targetAccountData),
                (err) => {
                  console.log(err);
                }
              );
              successMsg(`You have transferred $${amount} from ${sourceAccount} to ${targetAccount}`);
              operation();
            })
            .catch((err) => console.log(err));
        })
...
function checkAmount(amount, accountData) {
  if (!amount) {
    errorMsg("An error occurred. Please try again later.");
    return false;
  }
